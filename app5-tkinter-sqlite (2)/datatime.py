...
def connect():
    ...
    cur=conn.cursor()
    cur.execute("CREATE TABLE IF NOT EXISTS cancel (id INTEGER PRIMARY KEY, title text, author text, year datetime, isbn integer, time dateime)")
    conn.commit()
    conn.close()
...
def insert2(title,author,year,isbn, time):
    conn=sqlite3.connect("books2.db")
    cur=conn.cursor()
    cur.execute("INSERT INTO cancel VALUES (NULL,?,?,?,?,?)",(title,author,year,isbn, time))
    conn.commit()
    conn.close()
...
def search():
    conn=sqlite3.connect("books2.db")
    cur=conn.cursor()
    cur.execute(f"select * from book where datetime (year) > datetime('now')")
    rows=cur.fetchall()
    conn.close()
    return rows
print(search())
...
insert("oonnnnoo","John Smith","2020-01-13 23:24", 999999,now )
# # delete(3)
